...
/*  getStreamLocation() {
  ...
  void calculateAreaUsingCoordinates() {
    ...
    for (var i = 1; i < geoPlottingFarmList.length; i++) {
      ...
      var valueX = (double.parse(longitude) - double.parse(longitudeRef)) *
          circumference *
    ...
    num sqftArea = (areasSum / 0.00024711);
    ...
    //  num sqftAreaCalc = sqftArea;
    // var hectareVal = hectAcre;
    // var sqftdetHectCalc = detHect;

    addAreaItems(
        acre, hectAcre.toStringAsFixed(6), sqftArea.toStringAsFixed(6));
    /* addAreaItems((sqftArea * 0.000247105).toStringAsFixed(6),
        hectareVal.toString(), sqftAreaCalc.toString());*/
  }
  ...
  Future<bool> onBackPressed(BuildContext context) async {
    return (await Alert(
          ...
          title: TranslateFun.langList["cancls"] ?? "Cancel",
          desc: TranslateFun.langList["aryosuwatocacls"] ??
          ...
          buttons: [
            DialogButton(
              ...
              width: 120,
              child: Text(
                TranslateFun.langList["yescls"] ?? "Yes",
            ...
            DialogButton(
              ...
              width: 120,
              child: Text(
                TranslateFun.langList["nocls"] ?? "No",
                style: const TextStyle(color: Colors.white, fontSize: 20),
              ),
            )
          ],
            ...
            floatingActionButton: loading
                ? const Center(
                    child: CircularProgressIndicator(
                        color: Colors.green, strokeWidth: 2.5),
                  )
                : farmdata == null
                    ? Row(
