...
import '../Model/GeoAreacalucalteProposedLand.dart';
import '../Model/Geoareascalculate.dart';
import '../Model/GeoploattingproposedLandModel.dart';
...
List<GeoPloattingproposedLandModel> geoploattingproposedlist =[];
List<GeoareascalculateProposedLand> geoareascalculateproposed =[];
bool intermediateonlick = false;
bool endonclick = false;
bool startClick = false;
bool calculatearea = false;
GeoareascalculateProposedLand? data;
...
class geoploattingProposedLand extends StatefulWidget {
  @override
  geoploattingProposedScreen createState() => geoploattingProposedScreen();
}

class geoploattingProposedScreen extends State<geoploattingProposedLand> {
  @override
  void initState() {
    super.initState();
    geoploattingproposedlist.clear();
    geoareascalculateproposed.clear();
...
class _ManualmodegpsTabBarState extends State<ManualmodegpsTabBar> {
                ...
                child: new Column(children: <Widget>[
                  ...
                  Flexible(
                    child: ListView.separated(
                      primary: false,
                      shrinkWrap: true,
                      itemCount: geoploattingproposedlist.length,
                      itemBuilder: (BuildContext context, int index) {
                        return Dismissible(
                            key: ObjectKey(geoploattingproposedlist[index]),
                            ...
                            onDismissed: (direction) {
                              var item =
                                  geoploattingproposedlist.elementAt(index);
                              //To delete
                              deleteItem(index);
                            ...
                            child: Row(
                              ...
                              // this will take space as minimum as posible(to center)
                              children: <Widget>[
                                geoploattingproposedlist[index].State ==
                                        "Intermediate"
                                    ? new Expanded(
                                        child: Container(
                                          margin: const EdgeInsets.only(
                                          ...
                                          child: Text(
                                            geoploattingproposedlist[index]
                                                    .State +
                                                index.toString(),
                                ...
                                new Expanded(
                                  child: Container(
                                    ...
                                    child: Text(
                                      geoploattingproposedlist[index].Latitude,
                                      style: TextStyle(fontSize: 18),
                                    ),
                                  ),
                                  flex: 2,
                                ...
                                new Expanded(
                                  child: Container(
                                    ...
                                    child: Text(
                                      geoploattingproposedlist[index].Longitude,
                                      style: TextStyle(fontSize: 18),
                                    ),
                                  ),
                                  flex: 2,
        ...
        buttons: [
          DialogButton(
            ...
            onPressed: () {
              setState(() {
                Navigator.pop(dialogContext);
                if (geoploattingproposedlist[index].State == "Start") {
                  geoploattingproposedlist
                      .removeWhere((item) => item.State == "Start");
                  visibilityTag = true;
                  getLocation("Start", orderofGps);
                } else if (geoploattingproposedlist[index].State ==
                    "Intermediate") {
                  geoploattingproposedlist
                      .removeWhere((item) => item.State == "Intermediate");
                  visibilityTag = true;
                  getLocation("Intermediate", orderofGps);
                } else if (geoploattingproposedlist[index].State == "End") {
                  geoploattingproposedlist
          ...
          DialogButton(
            ...
            onPressed: () {
              setState(() {
                //  deletelatlng = true;
                Navigator.pop(dialogContext);
                geoploattingproposedlist.removeAt(index);
              });
            },
            color: Colors.deepOrange,
          ),
  ...
  addListItems(
    ...
    setState(() {
      GeoPloattingproposedLandModel data;
      data = new GeoPloattingproposedLandModel(Lat, Lng, State, OrderofGps);
      ...
      geoploattingproposedlist.add(data);
      print("objlocateListCHECK " + geoploattingproposedlist.length.toString());
    });
  }
  ...
  void undoDeletion(int index, GeoPloattingproposedLandModel item) {
    setState(() {
      geoploattingproposedlist.insert(index, item);
      visibilityTag = false;
      //_isVisible = false;
      if (geoploattingproposedlist.length > 0) {
        visibilityTag = true;
        //   _isVisible = true;
      }
    });
  ...
  void calculateAreaUsingCoordinates() {
    var latit = geoploattingproposedlist[0].Latitude;
    var longit = geoploattingproposedlist[0].Longitude;

    var intText = "";

    for (var j = geoploattingproposedlist.length - 2; j >= 1; j--) {
      print("decrementlistvalue");
      latit = geoploattingproposedlist[j].Latitude;
      longit = geoploattingproposedlist[j].Longitude;
      intText = intText + " " + latit + "," + longit;
    }
    //$("#interLoc").text("Intermediate:"+intText);

    latit =
        geoploattingproposedlist[geoploattingproposedlist.length - 1].Latitude;
    longit =
        geoploattingproposedlist[geoploattingproposedlist.length - 1].Longitude;
    ...
    var latitudeRef = geoploattingproposedlist[0].Latitude;
    var longitudeRef = geoploattingproposedlist[0].Longitude;

    print("longitudeRef12" + longitudeRef);
    print("latitudeRef12" + latitudeRef);

    for (var i = 1; i < geoploattingproposedlist.length; i++) {
      print("geoploattingfarmlist_location");
      var latitude = geoploattingproposedlist[i].Latitude;
      var longitude = geoploattingproposedlist[i].Longitude;
  ...
  addAreaItems(
    ...
    setState(() {
      ...
      geoareascalculateproposed.clear();
      data = new GeoareascalculateProposedLand(Acrea, Hectare, Squaremeter);
      geoareascalculateproposed.add(data!);
    });
  }
