...
class secondaryProcessing extends StatefulWidget {
  const secondaryProcessing({Key? key}) : super(key: key);

  @override
  State<secondaryProcessing> createState() => _CoffeePurchaseState();
}

class _CoffeePurchaseState extends State<secondaryProcessing> {
...
*/ /*when value is entered for reject 1299*/ /*
  ...
  Future<void> initvalues() async {
    List batchNoList = await db.RawQuery(
        'select distinct batchNo from batchCreationList where stockType ="2" and cast(weight as double)>0 and cast(noOfBag as int)>0 and isDelete = "0"');
  ...
  loadGrade(String batchNo) async {
    List inputGradeList = await db.RawQuery(
        'select b.grade,a.property_value,b.noOfBag,b.weight from batchCreationList b, animalCatalog a where batchNo = \'' +
            batchNo +
            '\' and a.DISP_SEQ=b.grade and stockType="2" and isDelete = "0" ');
  ...
  loadAvailableWtandKgs(String grade) async {
    List avlWtKg = await db.RawQuery(
        'select noOfBag,weight from batchCreationList where grade = \'' +
            ...
            '\' and batchNo = \'' +
            val_LotNo +
            '\' and stockType="2" and isDelete = "0" ');
  ...
  void saveCoffeePurchase() async {
    ...
    int custTransaction = await db.saveCustTransaction(
        txntime, datas.txnSecondaryProcessing, revNo, '', '', '');
    ...
    Alert(
      ...
      type: AlertType.info,
      title: "Transaction",
      desc: "Secondary Processing Successful",
      buttons: [
        DialogButton(
          child: Text(
            "OK",
